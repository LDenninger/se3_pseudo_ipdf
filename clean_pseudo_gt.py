...
if __name__=="__main__":
    ...
    parser.add_argument("-mode", type=int, default=0, help="Mode for validation: 0: Duplicate check, 1: Convergence validation")
    ...
    parser.add_argument("--clean", action="store_true", help="Use the already cleaned pseudo gt set")
    ...
    # Load the data
    data_loader = data.load_pls_dataset(config, return_pgt=True, cleaned_pgt=args.clean)


    # Run the validation
    if args.mode == 0:
        pose_labeling_scheme.run_duplicate_check(dataset=data_loader, hyper_param=config, angular_threshold=args.ang_th)
    else:
        ...
        pose_labeling_scheme.run_convergence_check(dataset=data_loader, obj_model=object_model, obj_model_sl=object_model_sl, hyper_param=config)
