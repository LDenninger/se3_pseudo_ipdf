...
def load_demo_dataset(hyper_param, poses=None, val_poses=None, validation_only=False):
    ...
    if not validation_only:
        data_train = data.YCBPoseDataset(obj_id=hyper_param["obj_id"][0],
                                remove_texture=not hyper_param["material"],
                                poses=poses)
...
def load_model_dataset(hyper_param, translation=False, validation_only=False):
    ...
    val_loader_list = [DataLoader(dataset=dat, batch_size=hyper_param['batch_size_val'], drop_last=True ,shuffle=True, num_workers=0) for dat in validation_datasets]
    if validation_only:
