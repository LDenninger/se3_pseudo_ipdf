...
def load_experiment(exp_name, args):
    ...
    save_path = exp_dir / "models" / (f"model_{args.rot_epoch}_{args.trans_epoch}.pth")

    torch.save(model.state_dict(), str(save_path))

    with open(str(exp_dir/"models"/"config_rotation.yaml"), 'w') as f:
            yaml.safe_dump(hyper_param_rot, f, default_flow_style=False)
    with open(str(exp_dir/"models"/"config_translation.yaml"), 'w') as f:
