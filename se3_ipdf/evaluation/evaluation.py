...
def visualize_output(model, dataset, save_dir,obj_id,
                        dataset_type="tabletop",
                        num_batch=1, 
                        num_displ=8,
                        displ_threshold=1e-4,
                        model_points = None,
                        mode=0,
                        pc_visualization=False,
                        est_visualization=False,
                        symsol=False,
                        return_img=False):

    """Visualize the output of the given model using the SO(3) Mollweide projection.
    In addition the image, the translation in the cartesian space and the observed pointloud
    with the overlayed estimated point cloud of the ground truth may be visualized.
    Arguments:
        model: model to be evaluated
        dataset: data used for the evaluation
        save_dir: directory the visualization are saved to
        num_batch: number of batches used during evaluation
        num_displ: number of images used for visualization in each batch
        mode:
            0: Visualize rotation model
            1: Visualize translation model
            2: Visualize SE(3) model
            3: Visualize ICP-model
        displ_threshold: Probability threshold used in the SO(3) Mollweide projection.
        translation_visualization: determines if the translation is visualized
        image_visualization: determines if the image is visualized
        train_mode: "supervised": ground truth matrix is used for visualization
                    "unsupervised": model estimates the ground truth through ICP
    """
    device = next(model.parameters()).device
    world_to_cam = Rotation.from_euler('XYZ', np.float32([0.1, 0.2, 0.3])).as_matrix()
    if symsol==True:
        progress_bar = tqdm(enumerate(dataset.as_numpy_iterator()), total=num_batch)
    else:
    ...
    for (step, batch) in progress_bar:
        ...
        img_org = batch['image_raw']
        img_full = batch['image_original']
        if dataset_type=="tabletop":
            depth_image = batch['depth_image']
        pose_gt = batch['obj_pose_in_camera'].to(device)

        rot_gt = batch['obj_pose_in_camera'][:,:3,:3].to(device)
        trans_gt = batch['obj_pose_in_camera'][:,:3,-1]

        if mode==0:
            query_rotation, prob_rot = model.output_pdf(img.float())
            prob_rot = torch.squeeze(prob_rot, dim=-1)
            query_rotation = query_rotation.cpu()
            prob_rot = prob_rot.cpu()
        elif mode==1:
            query_translation, prob_trans = model.output_pdf(img.float())
            ...
            pc_visualization=True
        #if translation_viz ==True:
        #    query_matrices = scale_to_original(query_matrices)
        #print(query_rot.shape)
        if pc_visualization==True:
            estimations = model.predict_rotation(img, gradient_ascent=True)

        img = img.cpu()

        img_full_list.append(img_full[:num_displ])
 
        for i in range(num_displ):
            #Visualize the used image
            img_viz = img_org[i]
            torchvision.utils.save_image(img_viz,os.path.join(save_dir, f"image_{step}_{i}.png"))
            img_viz = img_full[i]

            if dataset_type=="tless":
                img_viz = img_viz /255.
            torchvision.utils.save_image(img_viz,os.path.join(save_dir, f"image_full_{step}_{i}.png"))
            if dataset_type=="tabletop":
                img_viz = depth_image[i].unsqueeze(0) / 255.
                torchvision.utils.save_image(img_viz,os.path.join(save_dir, f"depth_image_{step}_{i}.png"))
                
            rot_gt_viz = rot_gt[i].cpu()

            if pc_visualization == True:
                assert model_points is not None
                visualize_transformations(model_points, pose_gt[i], estimations[i], filename = os.path.join(save_dir, f"visualization_pointcloud_{step}_{i}.png"))


            if est_visualization==True:
                visualize_so3_rotations(torch.repeat_interleave(estimations[i].unsqueeze(0), 2, dim=0).detach().cpu(),
                                        dataset=dataset_type, obj_id=obj_id,
                                        rotations_gt=rot_gt_viz,
                                        save_path=os.path.join(save_dir, f"estimation_{step}_{i}.png"))
            if mode==0 or mode==2:
                figure_rot = visualize_so3_probabilities(rotations=query_rotation[:,:3,:3], probabilities=prob_rot.detach()[i], rotations_gt=rot_gt_viz,
                ...
                img_rot_list.append(figure_rot)
            if mode==1 or mode==2:

                figure_trans = visualize_translation_probabilities(query_translation, prob_trans.detach()[i], trans_gt[i],
                                                        save_path=os.path.join(save_dir, f"visualization_trans_{step}_{i}.png"),
                                                        display_threshold_probability=displ_threshold,
                                                        return_img=return_img)
                img_trans_list.append(figure_trans)
       
        if return_img:
            return img_full_list, img_rot_list, img_trans_list
