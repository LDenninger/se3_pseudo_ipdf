...
if __name__=="__main__":
    ...
    if args.rot_epoch is not None and args.trans_epoch is not None:
        ...
        model = models.load_rotation_model(hyper_param_rot=hyper_param_rot, arguments=args)[0]

        dataset = data.load_dataset(hyper_param_rot, validation_only=True)

        visualizations.visualize_rotation_model(model=model, dataset=dataset, hyper_param=hyper_param_rot)
        ...
        model = models.load_translation_model(hyper_param_rot=hyper_param_trans, arguments=args)

        dataset = data.load_dataset(hyper_param_trans, validation_only=True)

        visualizations.visualize_translation_model(model=model, dataset=dataset, hyper_param=hyper_param_trans)[0]
